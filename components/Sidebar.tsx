...
const NAV_GROUPS: NavGroup[] = [
  ...
  },
  {
    label: 'Cost Control',
    items: [
      { label: 'Logged Tickets', href: '/tickets' },
      { label: 'Ticket Map', href: '/ticket-map' },
      { label: 'Purchase Orders', href: '/admin/pos' },
    ],
  },
  {
    label: 'Intake',
    items: [{ label: 'Upload & Reconcile', href: '/upload' }],
  },
  {
    label: 'Schedule',
    items: [
      { label: 'Ship Schedule', href: '/schedule' },
      { label: 'EWP Schedule', href: '/ewp-schedule' },
    ],
  },
];
      ...
      <nav className="flex-1 overflow-y-auto py-2">
        {NAV_GROUPS.map((group) => (
          <div key={group.label} className="mb-3 last:mb-0">
            <div className="px-4 pt-2 pb-1 text-[10px] uppercase tracking-widest text-enbridge-black/40 font-semibold">
              {group.label}
            </div>
              ...
              return (
                <Link
                  ...
                  className={`block px-4 py-1.5 text-[13px] transition-colors ${
                    active
                      ? 'bg-enbridge-black text-white font-medium'
